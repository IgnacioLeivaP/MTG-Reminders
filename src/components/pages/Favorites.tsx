    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-3">
          ...
          <h2 className="text-2xl font-bold dark:text-gray-100">{t.favorites.title}</h2>
        </div>
        {favorites.length > 0 && (
          <button
      ...
      {favorites.length === 0 ? (
        <div className="bg-white dark:bg-dark-card rounded-lg shadow-md p-6">
          <p className="text-gray-600 dark:text-gray-300">{t.favorites.empty}</p>
        </div>
      ) : (
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white dark:bg-dark-card rounded-lg shadow-xl p-6 max-w-sm w-full">
            <div className="flex items-center space-x-3 text-red-500 mb-4">
              ...
              <h3 className="text-lg font-semibold">{t.favorites.clearAll}</h3>
            </div>
            <p className="text-gray-600 dark:text-dark-text mb-6">{t.favorites.clearConfirm}</p>
            <div className="flex justify-end space-x-3">
              ...
              <button
                onClick={() => { clearFavorites(); setShowConfirmDialog(false); }}
